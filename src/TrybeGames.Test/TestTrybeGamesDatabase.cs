...
public class TestTrybeGamesDatabase
{
    ...
    {
        Player? playerById = databaseEntry.Players.Where(player => player.Id == playerIdEntry).FirstOrDefault();
        
        List<Game> gamesPlayedByPlayer = databaseEntry.GetGamesPlayedBy(playerById!);

        gamesPlayedByPlayer.Should().BeEquivalentTo(expected);
    }
